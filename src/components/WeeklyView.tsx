...
export default function WeeklyView({ events }: Props) {
    ...
    <div className="bg-white rounded-2xl shadow p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setWeekStart(w => subWeeks(w, 1))} className="text-gray-400 hover:text-gray-700 px-1 text-lg">‹</button>
          <span className="text-xs text-gray-500">{rangeLabel}</span>
          <button onClick={() => setWeekStart(w => addWeeks(w, 1))} className="text-gray-400 hover:text-gray-700 px-1 text-lg">›</button>
        </div>
        <button
      ...
      <div className="space-y-1">
        {days.map(day => {
          const de = weekEvents.filter(e => isSameDay(parseISO(e.start_at), day));
          const today = isToday(day);
          return (
            <div key={day.toISOString()} className={`rounded-xl px-3 py-2 ${today ? 'bg-indigo-50' : ''}`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                {de.length === 0 && <span className="text-xs text-gray-300">일정 없음</span>}
              </div>
              {de.length > 0 && (
                <div className="ml-16 space-y-1">
                  {de.map(e => (
                    <div key={e.id} className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-semibold shrink-0 ${MEMBER_TEXT[e.member] ?? 'text-gray-600'}`}>{e.member}</span>
